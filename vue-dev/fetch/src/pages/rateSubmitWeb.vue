     ...
     <ul class="list">
      ...
      <section class="content">
        <div class="content-item" v-for="(item,index) in list">
            <p class="item-title">
             {{item.name}}&nbsp;&nbsp;&nbsp;&nbsp;<span>小计：{{sectionScore[index]}}分</span>
            </p>
            <ul>
      ...
      <p class="allscore"><span>评课总分：{{allScore}} 分</span></p>
      <section class="comment-box">
        ...
        <div class="name-input"> 
            <p>评价人：<input type="text" v-model="person" placeholder="请输入你的姓名" /></p>
...
export default {
    ...
    computed: {
          ...
          let scoreArr=[];
           for(let i=0;i<this.list.length;i++){  
                ...
                for(let j=0;j<this.list[i].points.length;j++){
                    ...
                    total+=Number(this.list[i].points[j].score)
                }
                scoreArr[i]=total;
            }
            return scoreArr
        ...
        allScore(){
          let allScore=0;
          for(let i=0;i<this.sectionScore.length;i++){
              allScore+=Number(this.sectionScore[i])
          }
          return allScore;
        }
      },
    ...
    methods:{
        ...
        initData(){
            axios.post(baseUrl+'/web/course-valuation/term', JSON.stringify({
                id:this.params.id
              }))
        ...
        submit(){
            ...
            }else if(this.isNull(this.person)){
                  this.$toast("姓名不能为空！", {
                      durtaion: 200,
                      location: 'center' // 默认在中间
                });
            } else if(this.isEmojiCharacter(this.desc)||this.isEmojiCharacter(this.person)){
                 this.$toast("评论内容和姓名不能含有表情符号！", {
                      durtaion: 200,
                      location: 'center' // 默认在中间
                });
            }else{
              this.showLoading=true;
               axios.post(baseUrl+'/web/course-valuation/score', JSON.stringify({
