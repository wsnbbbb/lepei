<template>
  <van-form validate-first @failed="onFailed">
    <van-nav-bar title="参观登记" left-text="返回" @click-left="onClickLeft" left-arrow />
    <van-collapse v-model="activeNames" class="collapse">
      ...
      <van-collapse-item v-if="questList.length" name="5" class='collapse-item-last'>
        <template #title>
          <div class="title">
            <span>相关信息</span>
...
export default {
  ...
  methods: {
    ...
    async getDetail(applyId){
      ...
      console.log(params);
      let res = await getConsultPerson(JSON.stringify(params));
      console.log(res);
      if(res.success){
         this.detail.birthday=res.detail.birthday
         this.detail.childName=res.detail.childName
         this.detail.sex=res.detail.sex
         this.detail.homeAddress=res.detail.homeAddress
         this.activeNames=['1']
      }
    },
   ...
   async submitinfo() {
      ...
      if(!obj.childName||!obj.nickName||!obj.sex||!obj.birthday||!obj.nationality||!obj.motherTongue||!obj.homeAddress){
         flag=false
      }
      ...
      if (!flag) {
        ...
        params.uid = sessionStorage.getItem("uid");
        console.log(params);
        submitDetail(JSON.stringify(params)).then((res) => {
          // console.log(res);
          if (res.success) {
            this.$toast("提交成功");
            setTimeout(() => {
