...
export default {
    ...
    mounted(){
       if(this.$route.params.enter=="index"){
          if(this.$route.params.applyPersonId==this.personId){
    ...
    methods:{
        ...
        chartByRongYunWithHim(){
            ...
            if(this.isiOS()){
              window.webkit.messageHandlers.sendLocalMessage.postMessage({id: this.ryId, name: this.phone, portrait: this.portraitUrl});
            }else{
               window.app.sendLocalMessage(this.ryId,this.phone,this.portraitUrl); 
            } 
        },
        call(mobile){
          if(this.isiOS()){
