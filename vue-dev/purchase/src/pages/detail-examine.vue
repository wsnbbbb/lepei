<template>
    <div class="list-wrap">
         ...
         <section class="examine-apply">
          ...
          <div  class="examine-apply-info" v-if="isShowApplyPerson">
              ...
              <div class="apply-info-box">
                 <p class="apply-info-name">{{this.person}}</p>
                 <p class="apply-info-time">{{this.applyTime}}</p>
              </div>
              <span class="char-to-him" v-show="isShowCall"  v-on:click.stop="chartByRongYunWithHim()">
                <img src="../assets/icon-chat.png" alt="">
              </span>
               <span class="phone-to-him" v-show="isShowCall"  v-on:click.stop="call(list.mobile)">
       ...
       <comfirmAgree1 v-show="isShowAgree"  :childMsg="agreeMsg" @toparentevent="cancel()" @toparenteventcomfirm="comfirmAgree1"> </comfirmAgree1>
       <comfirmReject v-show="isShowReject"  @toparentevent="cancel()" @toparenteventcomfirm="comfirmReject1"></comfirmReject>
    </div>
</template>
<script>
import loading from '../components/loading'
import comfirmAgree1 from '../components/comfirmAgree1'
...
export default {
    ...
    mounted(){
        ...
        console.log(this.$route.params.applyPersonId)
    ...
    methods:{
       isiOS(){
        ...
        comfirmReceive(){
           this.showLoading=true;
          let _this=this;
          axios.post(baseUrl+'/teacher/material-purchase/confirm-receive', JSON.stringify({
                personId: this.user.personId,
                uid: this.user.uid,
                token:this.user.token,
                id:this.id
