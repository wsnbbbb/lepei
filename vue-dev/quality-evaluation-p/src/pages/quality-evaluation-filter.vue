<template>
    <div>
      ...
      <div class="person-box">
        <ul>
          <li class="person-li" v-bind:class="{ cannotSelect: list.status==1 }" v-for="list in personList"  v-bind:id="list.personId" v-on:click.stop="openUrl(list.personId,$event)" @touchstart="showSelectButton" @touchmove="clearLoop" @touchend="clearLoop" >
      ...
      <span class="s-all" id="s-all"  v-show="showComfirm" v-on:click.stop="selectAll($event)">全选</span>
      <a href="javascript:;" class="btn-submit" v-show="showComfirm" v-on:click.stop="comfirm">确定</a>
...
export default {
    ...
    methods:{
        ...
        init(type,status,classId){
            axios.post(baseUrl+'/teacher/quality-valuation/quality-class-list', JSON.stringify({
                        uid: this.user.uid,
                        token: this.user.token,
                        personId:this.user.personId,
        ...
        confirmFn(data){
            ...
            var obj=document.getElementsByClassName("person-li");
            for(var i=0;i<obj.length;i++){
                this.removeClass(obj[i],"selected");
                this.removeClass(obj[i],"select");
            }
             var selectBtn=document.getElementById("s-all");
             this.removeClass(selectBtn,"s-all-selected");
        },
        openPicker(){
            this.showMask=true;
            this.show1=true;
        },
        openUrl(studentId,event){
          if(this.showSelect!=true){
            var personArr=[];
            personArr.push(studentId)
            sessionStorage.setItem("personArr",personArr);
            this.$router.push({ path: "rateSubmit"});
          }else{
            if(this.hasClass(event.currentTarget,"selected")){
               this.removeClass(event.currentTarget,"selected");
             }else if(!this.hasClass(event.currentTarget,"cannotSelect")){
               this.addClass(event.currentTarget,"selected");
             }
            var obj=document.getElementsByClassName("person-li");
            var selectBtn=document.getElementById("s-all");
            var selectAll=true;
            for(var i=0;i<obj.length;i++){
              if((!this.hasClass(obj[i],"selected"))&&(!this.hasClass(obj[i],"cannotSelect"))){
                selectAll=false;
              }
            }
            if(selectAll!=true){
                ...
                this.addClass(selectBtn,"s-all-selected");
            }
          }
        ...
        showSelectButton() {
            ...
            _this.Loop=setTimeout(function(){
                ...
                var obj=document.getElementsByClassName("person-li");
                for(var i=0;i<obj.length;i++){
                  if(!_this.hasClass(obj[i],"cannotSelect")){
                    _this.addClass(obj[i],"select");
                  }
        ...
        selectAll(){
           if(this.hasClass(event.currentTarget,"s-all-selected")){
             this.removeClass(event.currentTarget,"s-all-selected");
           }else{
             this.addClass(event.currentTarget,"s-all-selected");
           }
          var obj=document.getElementsByClassName("person-li");
          for(var i=0;i<obj.length;i++){
            if(!this.hasClass(obj[i],"cannotSelect")){
              if(!this.hasClass(event.currentTarget,"s-all-selected")){
                this.removeClass(obj[i],"selected");
              }else{
                this.addClass(obj[i],"selected");
              }
            }
        ...
        comfirm(){
          ...
          for(var i=0;i<obj.length;i++){
           personArr.push(obj[i].getAttribute("id"))
          }
          sessionStorage.setItem('personArr',personArr);
...
.person-box ul>li.select{
  background: url(../assets/bg10.png);
  background-repeat: no-repeat;
  background-position: 1.5rem 0.41rem;
  background-size: 0.26rem 0.26rem;
  background-color: #fff;
}
.person-box ul>li.selected{
