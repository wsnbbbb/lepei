<template>
    <div>
    <!-- <div id="canvasItemLast" style="min-width:400px;height:400px"></div> -->

        <div class="part-title">
            <img class="img-l" src="../assets/bg23.png" alt="">
            <div class="part-r">
        ...
        <div class="part-item" v-for="(item,index) in itemArr" >
            <div class="part-title" v-on:click="openUrl('rateShow', item.id, item.label)">
                <img class="img-l" v-bind:src=item.icon alt="">
                <div class="part-r">
                  <p class="part-r-top">{{item.label}}</p>
                   <p class="part-r-bottom">{{item.epilogue}}</p>
                </div>
                <img class="img-r" src="../assets/b7.png" alt="">
            </div>
            <div class="g3-content">
              <!-- <p>学术成绩</p> -->
              <div class="g4-c-box">
         ...
         <div class="g3-content last-content" v-show="this.showLastGraph">
                <p>纵向对比</p>
                <div class="g4-c-box">
                    <div id="canvasItemLast" style="min-width:300px;height:300px"></div>
                </div>
        </div>
...
export default {
    ...
    methods:{
        ...
        previewAttach(url){
          console.log(url);
          if(isiOS()){
              window.webkit.messageHandlers.openClientUrl.postMessage(url);
          }else{
        ...
        openUrl(url, id, label){
          sessionStorage.setItem("quotasId", id)
        ...
        init(){
            ...
            axios.post(baseUrl+'/parent/quality-valuation/semester-list', JSON.stringify({
              uid: this.user.uid,
              token: this.user.token,
              personId: this.user.personId,
              // studentId:sessionStorage.getItem("personArr")
            ...
            .then(function (response) {
                ...
                if(response.data.success){
                        ...
                        this.initSemesterScore(semesterId);
                }else{
        ...
        initSynthetic(semesterId){
           axios.post(baseUrl+'/parent/quality-valuation/synthetic', JSON.stringify({
                            uid: this.user.uid,
                            token: this.user.token,
                            personId: this.user.personId,
        ...
        initQuota(semesterId){
                //获取指标 图表类型(1:柱状图,2:气泡图,3:曲线图,4:饼状图,5:雷达图)
                  axios.post(baseUrl+'/parent/quality-valuation/top-quota', JSON.stringify({
                          uid: this.user.uid,
                          token: this.user.token,
                          personId: this.user.personId,
                          semesterId: semesterId,
        ...
        initSubject(semesterId){
            //我的成绩
            axios.post(baseUrl+'/parent/quality-valuation/subject', JSON.stringify({
                uid: this.user.uid,
                token: this.user.token,
                personId: this.user.personId,
                semesterId: semesterId,
        ...
        initSemesterScore(semesterId){
            //我的成绩
            axios.post(baseUrl+'/parent/quality-valuation/semester-score', JSON.stringify({
                uid: this.user.uid,
                token: this.user.token,
                personId: this.user.personId,
                semesterId: semesterId,
            ...
            .then(function (response) {
                ...
                if(response.data.success){
                      var xcategories=[];
                      var yseries=[];
                      if(response.data.detail.scoreList.length<2){
                        this.showLastGraph=false;
                        return
                      }else{
                         this.showLastGraph=true;
                      }
                      for(let i=0;i<response.data.detail.scoreList.length;i++){
                        xcategories.push(response.data.detail.scoreList[i].name)
                        yseries.push(response.data.detail.scoreList[i].score)
                      }
                      yseries;
                      xcategories;
                      var chart3 = HighCharts.chart('canvasItemLast', {
                                chart: {
                                  type: 'areaspline'
                                },
                                credits: {
...
.g3-c-box ul li>div:first-child{
  border-top:1px solid #efefef;
}
.g3-c-box ul li:nth-child(3n)>div{
  border-right: none;
}
.g3-c-box ul{
  width: 100%;
