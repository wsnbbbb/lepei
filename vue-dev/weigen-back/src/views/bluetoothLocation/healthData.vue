		 ...
		 <el-tabs v-model="activeName">
			<el-tab-pane label="学生手环" name="first">
						...
						<el-row :gutter="24">
							...
							<el-col :span="3">
								<el-form-item  prop="gradeType">
									<el-select v-model="filters1.gradeType" placeholder="学业阶段"  @change="changeGradeType">
				...
				<el-table max-height="700" :data="studentRecord" highlight-current-row  style="width: 100%;">
					...
					<el-table-column prop="sport" label="步数/里程/卡路里" min-width="200" :show-overflow-tooltip="true">
						<template slot-scope="scope">
							<p class="step-p">{{scope.row.sportData.steps?scope.row.sportData.steps:'-'}}步/{{scope.row.sportData.distances?scope.row.sportData.distances/1000:'-'}}km/{{scope.row.sportData.calories?scope.row.sportData.calories:'-'}}卡</p>
							<p class="step-p">{{(scope.row.sportData.time)}}</p>
						</template>
					</el-table-column>
					<el-table-column label="最近心率" min-width="140" :show-overflow-tooltip="true">
						<template slot-scope="scope">
							<p class="step-p">{{scope.row.heartRateData.heartRate?scope.row.heartRateData.heartRate:'-'}}</p>
							<p class="step-p">{{scope.row.heartRateData.time}}</p>
						</template>
					</el-table-column>
					<el-table-column label="最近体温" min-width="140" :show-overflow-tooltip="true">
						<template slot-scope="scope">
							<p class="step-p">{{scope.row.temperatureData.temperature?scope.row.temperatureData.temperature:'-'}}</p>
							<p class="step-p">{{scope.row.temperatureData.time}}</p>
						</template>
					</el-table-column>
					<!-- <el-table-column label="最近睡眠" min-width="140" :show-overflow-tooltip="true">
						<template slot-scope="scope">
							<p class="step-p">{{scope.row.sleepData.totalSleep?scope.row.sleepData.totalSleep:'-'}}</p>
							<p class="step-p">{{scope.row.sleepData.time}}</p>
						</template>
					</el-table-column> -->
					...
					<el-table-column label="操作" width="220">
						<template slot-scope="scope">
							<el-button type="text" size="small" @click="toSecondPage(scope.row)">历史记录</el-button>
					</template>
					</el-table-column>
				</el-table>
				<!--工具条-->
...
<script>
	...
	import { getGrade,
	getClasses, getBuilds, getGradesByType, personHealth, getBraceletDetail, updateMac, sendUserInfo, batchSendUserInfo} from '../../api/api';
    ...
    name:"healthData",
		...
		methods: {
			toSecondPage (row) {
				this.$router.push({ path: '/health-history/' + row.personId });
			},
			// 重置
			resetForm (formName) {
					...
					if (valid) {
						...
						updateMac(params).then((res) =>{
							if(res.code == 200) {
								...
								personHealth(para).then((res) => {
									res.data.dataList.map(i=>{
										i.sportData.time = this.formatTime1(i.sportData.time)
										i.heartRateData.time = this.formatTime1(i.heartRateData.time)
										// i.sleepData.time = this.formatTime1(i.sleepData.time)
										i.temperatureData.time = this.formatTime1(i.temperatureData.time)
									})
