  ...
  <section>
    ...
    <div>
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column prop="devSn" label="基站ID" :show-overflow-tooltip="true"></el-table-column>
        <el-table-column prop="devName" label="基站名字"></el-table-column>
        <el-table-column prop="status" label="状态">
          <template slot-scope="scope">
            <span :style="{color:scope.row.status == 0 ? '#f00' : ('#606266')}">{{scope.row.status == 0 ? '离线' : (scope.row.status == 1 ? '在线' : '未知')}}</span>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="220">
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" @click="handleDel(1, scope.row)">删除</el-button>&emsp;
            <el-button type="text" size="small" @click="toSecondPage(scope.row)">历史记录</el-button>&emsp;
            <!-- <el-dropdown>
        ...
        <el-form :model="addDeviceData" ref="addDeviceData" :rules="rules" label-width="120px">
          <el-row :gutter="24">
            <el-col :span="20" :offset="1">
              <el-form-item label="基站ID：" prop="devSn">
                <el-select :disabled="selectDisable" v-model="addDeviceData.devSn" placeholder="请选择">
                 <el-option
            ...
            <el-col :span="20" :offset="1">
              <el-form-item label="基站名字：" prop="devName">
                <el-input clearable v-model="addDeviceData.devName" placeholder="请输入基站名字" auto-complete="off"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
      ...
      <el-row :gutter="20">
        <el-col :span="15" :offset="5">
          <p>
            ...
            <a class="downLoad" download='基站ID导入模板.xls'  :href="href" style="marginLeft:30px" >下载导入模板</a>
          </p>
        </el-col>
      </el-row>
...
import {getStations, getBuilds, getBaseStation, getPlaces, getStationDetail, deviceDetail,creatDevice,editDevice, deleteStation,devInit,copyDev, sswStationImport,  getStationDetail1, updateStation} from "../../api/api";
...
export default {
  data() {
    return {
      ...
      rules: {
        devName: [
          { required: false, message: '请输入基站名称', trigger: 'blur' }
        ],
        buildId: [
        ...
        devSn: [
          { required: true, message: '请选择基站ID', trigger: 'blur' }
        ],
        ip: [
  ...
  methods: {
    ...
    getAllDevice () {
      ...
      let params = {}
      getBaseStation(params).then((res) => {
        if(res.code == 200) {
    ...
    submitUpload() {
      ...
      sswStationImport(form).then(res => {
        if (res.code === 200) {
          if (res.data.error) {
    ...
    addDevice (type,index,row) {
      if(type == 1){
        this.drawerTitle = "添加"
        let params = {}
        getStationDetail1(params).then((res) =>{
          if(res.code == 200) {
         ...
         if(this.editType == 1) {
            creatDevice(params).then((res) =>{
              if(res.code == 200) {
                ...
                this.getStations(param)
                this.resetForm('addDeviceData')
                this.addDialog = false
              }
            })
          }else if(this.editType == 2) {
            params.devSn = this.oldDevSn
    ...
    // 1.本机人脸库 2.在线记录 
    toSecondPage (row) {
        this.$router.push({ path: '/station-history/' + row.devSn });
    },
  ...
  mounted() {
    ...
    this.getStations(params);
    this.href = util.getUpload('基站ID导入模板.xls')
  },
};
</script>
