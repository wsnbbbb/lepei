		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				...
				<el-form-item class="query-btn">
					<el-button type="primary" size="medium"  v-on:click="entryDataRecords">查询</el-button>
					<!-- <el-button type="primary" size="medium"  v-on:click="play(3)">play</el-button> -->
				</el-form-item>
			</el-form>
		</el-col>
		...
		<el-col :span="24" class="ul-col">
			<li class="items items1" v-bind:class="{ putIn: showKeyframe}" v-show="showKeyframe">
				<div class="img-box">
					...
					<p class="tag3" v-show="newData.isApplyLeave == 1">
						<img src="../../assets/icon-time.png" alt="">&nbsp;&nbsp;请假
					</p>
					<p class="tag2" v-bind:class="{ blue: (newData.personType==1)&&newData.inResidence==1, orage: newData.personType==1&&newData.inResidence==2, red: newData.personType==5}">
						{{(newData.personType==1)?(newData.inResidence==1?"住读":"走读"):newData.personType==2?"教师":(newData.personType==5?"家长":"职工")}}
					</p>
				</div>
				<p class="info-top">
					<span class="person-name">{{newData.personName}}{{newData.personType==5?newData.relationName:""}}</span>
					<span class="entry-type">{{newData.entryType==1?"进校":"出校"}}</span>
				</p>
				<p v-show="(newData.personType==1||newData.personType==5)">{{newData.gradeName}}&nbsp;&nbsp;{{newData.className}}</p>
			...
			<div class="ul-wrap">
				<li class="items" :key="index" v-for="(item, index) in list">
					<div class="img-box">
						...
						<p class="tag3" v-show="item.isApplyLeave == 1">
							<img src="../../assets/icon-time.png" alt="">&nbsp;&nbsp;请假
						</p>
						<p class="tag2" v-bind:class="{ blue: item.personType==1&&item.inResidence==1, orage: item.personType==1&&item.inResidence==2, red: item.personType==5}">
							{{item.personType==1?(item.inResidence==1?"住读":"走读"):item.personType==2?"教师":(item.personType==5?"家长":"职工")}}
						</p>
					</div>
					<p class="info-top">
						<span class="person-name">{{item.personName}}{{item.personType==5?item.relationName:""}}</span>
						<span class="entry-type">{{item.entryType==1?"进校":"出校"}}</span>
					</p>
					<p v-show="item.personType==1||item.personType==5">{{item.gradeName}}&nbsp;&nbsp;{{item.className}}</p>
...
<script>
	...
	let audio7 = require('../../../static/voice/请假学生.mp3')
	let audio8 = require('../../../static/voice/家长卡.mp3')

	import { getUserListPage, getAllDevices, getPersonByChipNo, entryDataRecords, createDevice, getEntryMacList, getBuilds, getPlaces, getDeviceList, 
	removeUser, batchRemoveUser, editUser, addUser , getRecord, checkOrder, cancelCheckOrder, getSocketServerules} from '../../api/api';
	...
	export default {
		...
		methods: {
			...
			getSocketServerules() {
				let _this = this
				getSocketServerules({}).then((res) => {
					if(res.code==200){
						this.socketUrl = 'ws://' + `${res.data.realTimeSocket}`
						_this.socket();
					}
				});
			},
					...
					if(res.code == 200){
						if(Array.isArray(res.data)){
							...
							_this.options.map(item=>{
								if(item.devSn == obj.devSn){
									devName = item.devName
								}
							})
							let object = Object.assign({
								isApplyLeave: obj.isApplyLeave.toString(),
								devName: devName,
								entryType: obj.direct.toString(),
								status: obj.valid == 1 ? 0 : 1,
							 	entryTime: new Date(obj.dateTime).getTime()/1000}, res.data);
							_this.newData = object
							console.log("newData",_this.newData)
							if(obj.valid == 0){
								...
								if(res.data.personType == 1){//学生
									console.log("请假",obj.isApplyLeave)
									if(obj.isApplyLeave == 1){
										_this.play(7)
									}else if(res.data.inResidence == 1){//1: 住读, 2: 走读
										_this.play(2)
									}else{
										_this.play(6)
									}
			...
			play(type) {
				...
				switch (type) {
					...
					case 6:
						...
						break;
					case 7:
						buttonAudio.setAttribute('src', audio7)
						break;
					case 8:
						buttonAudio.setAttribute('src', audio8)
						break;
					default:
						break;
				}
			...
			socket() {
				...
				this.ws.onmessage = function (evt) {
					...
					if(received_msg.cardNo){
						if(_this.value1.length==0&&_this.devSns.includes(received_msg.devSn.toString())){
							_this.getPersonByChipNo(received_msg)
							console.log("发请求")
						}else if(_this.value1.length>0&&_this.value1.includes(received_msg.devSn.toString())){
							_this.getPersonByChipNo(received_msg)
							console.log("发请求")
						}
					}
