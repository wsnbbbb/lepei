	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				...
				<el-form-item>
					<el-button type="primary" size="medium"  @click="handleSet">图片路径</el-button>
				</el-form-item>
			</el-form>
		</el-col>
		...
		<el-table :data="users" highlight-current-row @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column label="操作" width="150">
				<template slot-scope="scope">
					<el-button type="text" size="small" @click="handleOpen(scope.$index, scope.row)">远程开门</el-button>
		...
		<el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false">
			...
			<div slot="footer" class="dialog-footer">
				...
				<el-button type="primary" size="medium"  @click.native="updateDevice">提交</el-button>
			</div>
		</el-dialog>

		<!--新增界面-->
		<el-dialog title="添加终端" :visible.sync="addFormVisible" :close-on-click-modal="false">
		...
		<!--设置界面-->
		<el-dialog title="门禁抓拍图片存储路径" :visible.sync="setFormVisible" :close-on-click-modal="false">
			...
			<div slot="footer" class="dialog-footer">
				<el-button size="medium" @click.native="setFormVisible = false">取消</el-button>
				<el-button type="primary" size="medium"  @click.native="setSubmit" >提交</el-button>
			</div>
...
<script>
	...
	import { openDevice, showPicSet, savePicSet, getCameraList, updateDevice, entryMacDetail, createDevice, deleteDevice, getEntryMacList, getBuilds, getPlaces, getDeviceList } from '../../api/api';
    ...
    name:"devConfig",
		...
		methods: {
			...
			handleOpen: function (index, row) {
				this.$confirm('请在确保安全的情况下，使用远程开门功能，是否确认开门?', '提示', {
					type: 'warning'
				}).then(() => {
					this.openFullScreen()
					let para = { devSn: row.devSn };
					openDevice(para).then((res) => {
						this.$message({
							message: '操作成功',
							type: 'success'
						});
				...
				entryMacDetail(param).then((res) => {
					if(res.code==200){
						...
						if(res.data.passwords.length>=1){
							...
							this.$set(this.editForm,'password1',res.data.passwords[0])
							this.$set(this.editForm,'password2',res.data.passwords[1])
							this.$set(this.editForm,'password3',res.data.passwords[2])
							this.$set(this.editForm,'password4',res.data.passwords[3])
						}else{
							this.passwords1 = "1"
						}
			...
			handleSet: function () {
				let para = { };
					showPicSet(para).then((res) => {
						// this.$message({
						// 	message: '删除成功',
						// 	type: 'success'
						// });
					this.$refs['setForm']&&this.$refs['setForm'].resetFields();
					this.setForm.picType = res.data.picType==0?"":res.data.picType
