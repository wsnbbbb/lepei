<template>
    <div>
      ...
      <div class="item"> 
        <img src="../assets/icon-a7.png" alt="">
        存入金额
         <input type="text" class="input-money" placeholder="请输入"  v-model="money">
        <!-- <span class="item-arr"></span> -->
      </div>

      <a href="javascript:;" class="login-btn save-btn" v-on:click.stop='save'>存入</a>
      ...
      <div class="result-mask-box" v-show="showSuccess" @touchmove.prevent>
          <div class="result-mask">
              <img src="../assets/icon-a8.png" alt="">
              <p>
                <span>{{name}}</span>，<span>{{className}}</span><br/>
                存入<span class="green">{{showMoney}}</span>元，当前存款总计<span class="green">{{totalMoney}}</span>元
              </p>
      ...
      <div class="result-mask-box result-mask-box-fail" v-show="showFail" @touchmove.prevent>
          <div class="result-mask">
              <img src="../assets/icon-a9.png" alt="">
              <p>
                温馨提示：<br/>
                <span>{{failText}}</span>
              </p>
...
export default {
       ...
       axios.post(baseUrl+'/pad/financial/class', JSON.stringify({
        ...
        .then(function (response) {
            ...
            if(response.data.success){
             ...
             this.pickData1.pData1=pickerArr;
    ...
    methods:{
        ...
        save(){
            ...
            this.showLoading=true;
            axios.post(baseUrl+'/pad/financial/access', JSON.stringify({
                uid: sessionStorage.getItem("uid"),
                token: sessionStorage.getItem("token"),
              ...
              .then(function (response) {
                this.showLoading=false;
                  ...
                  this.showMoney=this.fmoney(this.money);
                }else{
                  this.showFail=true;
