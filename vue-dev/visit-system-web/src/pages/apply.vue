              ...
              <div class="item">
                ...
                <div>
                  <p class="visit-common">
                   拜访对象
                    <span v-on:click="search" class="i-right">
                      {{teacherName}}&nbsp;&nbsp;
                      {{telephone&&telephone.replace(/(\d{3})\d{4}(\d{4})/, '$1****$2')}} 
                  ...
                  <div class="visit-common visit-pic">
                    照片
                    <label class="ui-upload" v-show="isShowUpload">
                      <form id="pic-form">
                        <input type="file" id="file" @change="changepic" accept="image/*" style="display: none;" />
                      </form>
                    </label>
...
export default {
    ...
    mounted(){
      this.getQiNiuToken();
      this.visitorTel = sessionStorage.getItem("phone")
    },
    beforeDestroy () {
    ...
    methods:{
      ...
      searchName(){
        ...
        axios.post(baseUrl+'/web/visit-records/search', JSON.stringify({
      ...
      apply(){
        var _this = this
        this.showLoading=true;
        axios.post(baseUrl+'/web/visit-records/apply', JSON.stringify({
            schoolId: this.$route.params.schoolId,
            telephone: sessionStorage.getItem("phone"),
      ...
      showDatePicker (demo, type) {
        ...
        this.$datepicker(dataObj).then((e) => {
          console.log(e)
           _this.visitDate = e[0]+"年"+e[1]+"月"+e[2]+"日"+" "+e[3]+":"+(e[4]<10?("0"+e[4]):e[4])+"分"
