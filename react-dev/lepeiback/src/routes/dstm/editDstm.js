...
import {isBlank, formatDate, duplicateRemoveArr, getQueryString} from '../../utils/public';
import './style.less';
...
class EditDstm extends Component{
    constructor(props) {
        super(props);
        this.state = {
          visible: false,
    ...
    dealData = (arrData)=>{
      ...
      arrData.map((item, index)=>{
        let arrImg = []
        item.images&&item.images.map((i, idx)=>{
          arrImg.push({
            uid: idx,
            url: getImg(i),
            isOld: true,
      ...
      return arr.length==0?[{selfId: 1,content: '',images: []}]: arr
    }
    getDetail=()=> {
      // this.setState({selectedRowKeys: this.state.groupItem[index].selectIds})
      this.props.dispatch({
        type:'dstm/getApplyDetail',
        payload: {
        ...
        callback: res=>{
          if(res.code==200){
              this.setState({
                ...
                groupType: Number(res.data.groupType),
                typeId: res.data.typeId,
                background: this.dealData(res.data.background.description),
                leaders: res.data.leaders,
                plan: this.dealData(res.data.plan.description),
                prepare: this.dealData(res.data.prepare.description),
                process: this.dealData(res.data.process.description),
                purpose: this.dealData(res.data.purpose.description),
                report: this.dealData(res.data.report.description),
              })
                      ...
                      
                      {
                        background&&background.map((item, index)=>{
                          return  <span key={index}>
                                    <TextArea rows={4} value={item.content}  
                                    placeholder="请输入背景"
...
export default connect(mapStateToProps)(Form.create()(EditDstm));
