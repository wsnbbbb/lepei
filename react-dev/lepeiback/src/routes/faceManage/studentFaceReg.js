...
class StudentFaceReg extends Component{
    constructor(props) {
        super(props);
        this.state = {
          faceRegList:[],
    ...
    cancel = (id) =>{
      let _this = this
      this.props.dispatch({
        type: 'faceManage/personFaceCancel',
        payload: {"personId": id},
        callback: res=>{
          if(res.code===200){
    ...
    personBatchReg=()=>{
      ...
      _this.props.form.validateFields((err, values) => {
        ...
        const params={
          ...
          "kw": values.kw||'',
          "gradeId": values.gradeId?values.gradeId.substring(values.gradeId.lastIndexOf('-')+1, values.gradeId.length):'',
          "classId": _this.state.classValue?_this.state.classValue.substring(_this.state.classValue.lastIndexOf('-')+1, _this.state.classValue.length):'',
          "headStatus": values.headStatus||'',
          "faceStatus": values.faceStatus||'',
          "cardStatus": values.cardStatus||'',
        }
        _this.props.dispatch({
          type: 'faceManage/personBatchReg',
          payload:params,
          callback: res=>{
              console.log(res.msg)
          }
    ...
    batchCancel = ()=>{
      ...
      this.props.form.validateFields((err, values) => {
        ...
        this.props.dispatch({
          type: 'faceManage/personBatchCancel',
          payload:params,
        })
        document.getElementById('ajaxLoading').style.display = 'none';
        let loading = document.getElementById('ajaxLoading2');
                ...
                <Row gutter={24}>
                  ...
                  <Col span={5}>
                    <FormItem {...formItemLayout} label={'班级'}>
                        <Select showSearch placeholder="请选择" value={this.state.classValue||undefined} onChange={this.classChange} disabled={this.state.disabled}>
                          {classOptions}
                        </Select>
                    </FormItem>
                  </Col>
...
export default connect(mapStateToProps)(Form.create()(StudentFaceReg));
