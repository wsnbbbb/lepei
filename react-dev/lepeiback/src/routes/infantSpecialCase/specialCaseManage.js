...
class SpecialCaseManage extends Component{
    constructor(props) {
        super(props);
        this.state = {
          page: 1,
    ...
    getList = (params) =>{
      this.props.dispatch({
        type:'infantSpecialCase/getSpecialCase',
        payload:params,
        callback:res =>{
          if(res.code === 200){
            this.setState({
    ...
    add = (type,id) => {
      if(type == 1){
        this.props.dispatch(routerRedux.push("/special-case-detail"))
      }else{
        this.props.dispatch(routerRedux.push("/special-case-detail?id=" + id))
      }
    }
    // 重置
    reset = () => {
    ...
    showConfirm = (id) => {
      ...
      confirm({
        ...
        onOk() {
          me.props.dispatch({
            type:'infantSpecialCase/delSpecialCase',
            payload:{ id },
            callback:(res) =>{
              if(res.code === 200){
    ...
    goToDetail = (type,id) =>{
      if(type == 1){
        this.props.dispatch(routerRedux.push("/special-case-detail?type=" + type))
      }else{
        this.props.dispatch(routerRedux.push("/special-case-detail?id=" + id + "&type=" + type))
      }
    }
    render(){
...
export default connect(mapStateToProps)(Form.create()(SpecialCaseManage));
