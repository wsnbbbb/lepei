...
class visitRegister extends Component {
	constructor(props) {
		super(props);
		this.state = {
			page: 1,
	...
	getList = (params) => {
		this.props.dispatch({
			type: 'consultEntrance/getVisitList',
			payload: params,
			callback: (res) => {
				if (res.code == 200) {
					this.setState({
	...
	showConfirm = (id) => {
		...
		confirm({
			...
			onOk() {
				me.props.dispatch({
					type: 'consultEntrance/delVisitList',
					payload: { id: id },
					callback: (res) => {
						if (res.code === 200) {
							...
							me.props.form.validateFields((err, values) => {
								const params = {
									...
									submitStartTime:
										(values.time &&
											values.time[0] &&
											toTimestamp(moment(values.time[0]).format('YYYY-MM-DD'), true)) ||
										'',
									submitEndTime:
										(values.time &&
											values.time[0] &&
	...
	//跳转详情
	toDetail = (id) => {
		this.props.dispatch(routerRedux.push('/visitRegister-detail?id=' + id));
	};
	...
	export = () => {
		this.props.form.validateFields((err, values) => {
			...
			let kw = values.kw || '';
			let hasFeedback = values.hasFeedback || '';
			let submitStartTime =
				(values.time && values.time[0] && toTimestamp(moment(values.time[0]).format('YYYY-MM-DD'), true)) || '';
			let submitEndTime =
				(values.time && values.time[1] && toTimestamp(moment(values.time[1]).format('YYYY-MM-DD'))) || '';
			let url = portUrl(
				'/manager/enrolment/visit/records/export?userId=' +
					userId +
					'&userType=' +
					userType +
					'&accessToken=' +
			...
			<div className="visitRegister">
				{/* <div className="breadcrumb">
					<Breadcrumb>
						...
						<Breadcrumb.Item>参观登记表</Breadcrumb.Item>
					</Breadcrumb>
					<h3>参观登记表</h3>
				</div> */}
				<div className="content-main">
					<Form className="content-form">
						<Row gutter={24}>
							<Col span={6}>
								<FormItem label="">
									{getFieldDecorator('kw')(<Input allowClear placeholder="幼儿姓名/家庭住址/反馈信息" />)}
								</FormItem>
							</Col>
							<Col span={4}>
								<FormItem>
									{getFieldDecorator('hasFeedback')(
										<Select allowClear placeholder="有无反馈">
											<Option value="">全部</Option>
											<Option value={1}>有反馈</Option>
											<Option value={2}>无反馈</Option>
										</Select>
									)}
								</FormItem>
							</Col>
							...
							<Col span={8}>
								<FormItem>
									{getFieldDecorator('time')(
										<RangePicker allowClear format={'YYYY/MM/DD'} onChange={this.onTimeChange} />
									)}
								</FormItem>
							</Col>
					...
					<Table
						...
						scroll={{ x: 1000 }}
						columns={columns}
						dataSource={visitList.dataList}
						pagination={false}
					/>
					<PageIndex
						getPage={this.onPageChange.bind(this)}
						total={visitList.totalCount}
						totalPage={visitList.totalPage}
						currentPage={visitList.currentPage}
					/>
				</div>
				<Drawer
					title="配置"
							...
							<Col span={24}>
								<Form.Item {...formItemLayout} label="请选择关联表格">
									{getFieldDecorator('formId', {
										initialValue:
											itemListId.indexOf(formId.toString()) > -1 ? formId.toString() : ''
									})(
										<Select
											allowClear
											placeholder="不选择"
...
export default connect(mapStateToProps)(Form.create()(visitRegister));
