...
class OutsideApplyRecord extends Component {
  constructor(props) {
    super(props);
    this.state = {
      page: 1,
  ...
  getTypeList = () => {
    this.props.dispatch({
      type:'outsideApply/outsideTypeList',
      callback:(res)=>{
          if(res.code === 200){
  ...
  getList = (params) => {
    this.props.dispatch({
      type:'outsideApply/outsideApplyRecord',
      payload: params,
      callback: res => {
        if(res.code == 200){
          this.setState({
  ...
  checkDetail = (id) => {
    this.props.dispatch({
      type:'outsideApply/outsideApplyDetail',
      payload:{id},
      callback:(res)=>{
          if(res.code === 200){
            let answerContents = []
      ...
      <div className="special-case-record">
        <div className="breadcrumb">
          <Breadcrumb>
            ...
            <Breadcrumb.Item>校外状况记录</Breadcrumb.Item>
          </Breadcrumb>
          <h3>校外状况记录</h3>
        </div>
              ...
              itemContents && itemContents.map(item =>{
                if(item.type == 4){
                  return <div key={item.seq}>
                          ...
                          <p>{item.answers&&item.answers[0]&&item.answers[0].item + '：' + item.answers&&item.answers[0]&&item.answers[0].val}</p>
                        </div>
                }else if(item.type == 5){
                  return <div key={item.seq}>
                            <h4>{item.seq + '、' + item.title}</h4>
                            {
                              item.answers&&item.answers.map((v,i) =>{
...
export default connect(mapStateToProps)(Form.create()(OutsideApplyRecord));
