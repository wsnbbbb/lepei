...
class AuditorSet extends Component{
  constructor(props) {
      super(props);
      this.state = {
  ...
  componentDidMount=()=>{
    ...
    this.getAuditorDetail()
  }
  
  // 获取处理人设置详情
  getAuditorDetail = () =>{
    this.props.dispatch({
      type:'assetsManage/getAuditorDetail',
      callback:(res) =>{
        if(res.code === 200){
          ...
          this.setState({
            ...
            thirdExaminer:res.data.personIds.thirdExaminer,
  ...
  save = () =>{
    ...
    const params = {
      ...
      thirdExaminer:this.state.thirdExaminer,
      showPersons:this.state.showPersons,
    }
    console.log({params})
    this.props.dispatch({
      type:'assetsManage/saveAuditorSet',
      payload:params,
      callback:(res) =>{
        if(res.code === 200){
          window.history.go(-1)
  ...
  render(){
    const { firstExaminerData, secondExaminer, thirdExaminer, showPersons } = this.state;
    ...
    return (
        <div className="content-main handler-set">
          <Breadcrumb className="Breadcrumb">
            <Breadcrumb.Item><Link to="/goods-apply">物品申购</Link></Breadcrumb.Item>
          ...
          <Form className="form">
            ...
            <Row gutter={24}>
              <Col span={9} >
                <FormItem {...formItemLayout} label='第三审批人'>
                  <Select
...
export default connect(mapStateToProps)(Form.create()(AuditorSet));
