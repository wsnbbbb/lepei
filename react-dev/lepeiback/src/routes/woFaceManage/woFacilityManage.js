...
class WoFacilityManage extends Component{
    constructor(props) {
        super(props);
        this.state = {
          dataList:[],
    ...
    facilityList = (params) =>{
      this.props.dispatch({
        type:'woFaceManage/facilityList',
        payload:params,
        callback:(res) =>{
          if(res.code === 200){
    ...
    clearFacility = (id) =>{
      ...
      confirm({
        ...
        onOk() {
          that.props.dispatch({
            type:'woFaceManage/clearFacility',
            payload:{"deviceKey":id},
            callback:(res)=>{
              if(res.code===200){
    ...
    // 授权管理
    authorizeManage = (id) =>{
      this.props.dispatch(routerRedux.push("/wo-authorize-manage?deviceKey="+id))
    }
    render(){
...
export default connect(mapStateToProps)(Form.create()(WoFacilityManage));
