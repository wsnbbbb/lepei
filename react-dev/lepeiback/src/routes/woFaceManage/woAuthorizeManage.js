...
class WoAuthorizeManage extends Component{
    constructor(props) {
        super(props);
        this.state = {
          page:1,
    ...
    deviceDetail = (params) =>{
      ...
      this.props.dispatch({
        type:'woFaceManage/checkDeviceStatus',
    ...
    // 添加
    add = () =>{
      this.props.dispatch(routerRedux.push("/wo-unauth-person?deviceKey="+this.state.deviceKey))
    }
    // 导出
    export=()=>{
      this.props.form.validateFields(["kw","personType","authStatus","headStatus"],(err, values) => {
        ...
        let url=portUrl("/manager/woface-device/auth-list-export?userId="+userId+"&userType="+userType+"&accessToken="+token+
    ...
    copyAuth = () =>{
      this.props.dispatch({
        type:'woFaceManage/getAllDevice',
        callback:(res) =>{
          if(res.code === 200){
            this.setState({
              allDevice:res.data,
        ...
        return (
          <div className="authorize-manage">
            <Breadcrumb style={{margin:"10px 0"}}>
              <Breadcrumb.Item><Link to="/wo-facility-manage">设备管理</Link></Breadcrumb.Item>
...
export default connect(mapStateToProps)(Form.create()(WoAuthorizeManage));
