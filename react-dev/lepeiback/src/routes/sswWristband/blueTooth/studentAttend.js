...
import {getSexType,getGradeType,getResidence,getBeforeDate} from '../../../utils/public';
...
class StudentAttend extends Component{
    ...
    export=()=>{
      ...
      this.props.form.validateFields((err, values) => {
        ...
        let url=portUrl("/manager/attend-records/export?personType=1&userId="+userId+"&userType="+userType+"&accessToken="+token+
          "&kw="+kw+"&gradeId="+gradeId+"&buildId="+buildId+"&classId="+classId+"&status="+status+"&startDate="+startDate+
          "&endDate="+endDate+"&sectionId="+sectionId+"&roomId="+roomId)
    ...
    render(){
        const columns = [{
          ...
          const {attendData,gradeId,classValue,placeDisabled,classDisabled,startDate,endDate} = this.state;
          console.log(startDate,endDate)
          const {commonGradeData,classNameData,buildingList,placeList,currentSections} = this.props;
          if(!attendData){
            return null;
          }
          let gradeChild=[]
...
export default connect(mapStateToProps)(Form.create()(StudentAttend));
