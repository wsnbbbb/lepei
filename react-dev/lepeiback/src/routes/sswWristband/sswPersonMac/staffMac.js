...
class StaffMac extends Component{
    constructor(props) {
        super(props);
        this.state = {
    ...
    personMacLists=(params)=>{
      this.props.dispatch({
        ...
        callback:(res)=>{
            if(res.code===200){
                this.setState({staffMacList:res.data})
            }
        }
      })
    }
    ...
    goToDetail=(type,id)=>{
      this.props.dispatch(routerRedux.push("/bind-staff-mac"))
    }
          ...
          this.props.dispatch({
            ...
            callback:(res)=>{
              if(res.code===200){
                ...
                const params={
                  "page":this.state.page,"prePage":this.state.prePage,"personType":3
                }
                this.personMacLists(params)
                this.props.form.resetFields(['personName','macId']);
              }
    ...
    export=()=>{
      this.props.form.validateFields((err, values) => {
        ...
        let url=portUrl("/manager/shunshiwei-person-mac/export?personType=3&userId="+userId+"&userType="+userType+"&accessToken="+token+
    ...
    render(){
        const columns = [{
          ...
          const {staffMacList,personMacDetail} = this.state;
          if(!staffMacList){
            return null
          }
          let classOptions=[];
        ...
        return (
            <div className="content-main">
              ...
              <Table className='content-table' scroll={{ x: 1000 }} columns={columns} dataSource={staffMacList.dataList} pagination={false}/>
              <PageIndex getPage={this.onPageChange.bind(this)} total={staffMacList.totalCount} totalPage={staffMacList.totalPage} currentPage={staffMacList.currentPage}/>
              <Modal
                title="修改"
...
export default connect(mapStateToProps)(Form.create()(StaffMac));
