...
class TractList extends Component{
    constructor(props) {
        super(props);
        this.state = {
        ...
        this.props.dispatch({
            ...
            callback:(res)=>{
                if(res.code===200){
                    ...
                    this.props.dispatch({
                      type:'sswWristband/getSswTrackList',
                      payload:{
                        "macId":this.state.macId,"page":1,"prePage":20,"date":date
                      },
                      callback:(res)=>{
                        if(res.code===200){
                          this.setState({sswTrackList:res.data})
                        }
                      }
                    })
                }
    ...
    render(){
        const columns = [{
          ...
          const {modalData,infoData,date,sswTrackList} = this.state;
        ...
        return (
            <div className="content-main">
             ...
             {sswTrackList?<Table className='content-table' scroll={{ x: 1000 }} columns={trackColumns} dataSource={sswTrackList.dataList} pagination={false}/>:null}
             {sswTrackList?<PageIndex getPage={this.onPageChange.bind(this)} total={sswTrackList.totalCount} totalPage={sswTrackList.totalPage} currentPage={sswTrackList.currentPage}/>:null}
              <Modal
                title="请选择"
...
const mapStateToProps = (state) => {
  return {
    ...
    // sswTrackList:state.sswWristband.sswTrackList,
  }
}
export default connect(mapStateToProps)(Form.create()(TractList));
