...
class HeartList extends Component{
        ...
        this.props.dispatch({
            ...
            callback:(res)=>{
                if(res.code===200){
                    ...
                    this.props.dispatch({
                      type:'sswWristband/getSswHeartList',
                      payload:{
                        "macId":this.state.macId,"page":1,"prePage":20,"date":date
                      },
                      callback:(res)=>{
                        if(res.code===200){
                          this.setState({sswHeartList:res.data})
                        }
                      }
                    })
                }
    ...
    render(){
        const columns = [{
          ...
          const {modalData,infoData,date,sswHeartList} = this.state;
        ...
        return (
            <div className="content-main">
              ...
              {sswHeartList?<Table className='content-table' scroll={{ x: 1000 }} columns={heartColumns} dataSource={sswHeartList.dataList} pagination={false}/>:null}
              {sswHeartList?<PageIndex getPage={this.onPageChange.bind(this)} total={sswHeartList.totalCount} totalPage={sswHeartList.totalPage} currentPage={sswHeartList.currentPage}/>:null}
              <Modal
                title="请选择"
...
const mapStateToProps = (state) => {
  return {
    ...
    // sswHeartList:state.sswWristband.sswHeartList,
  }
}
export default connect(mapStateToProps)(Form.create()(HeartList));
