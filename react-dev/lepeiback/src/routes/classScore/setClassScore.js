...
class SetClassScore extends Component{
    ...
    showConfirm=(id)=> {
      ...
      confirm({
        ...
        onOk() {
          me.props.dispatch({
            type:'classScore/delClassScoreType',
            payload:{"typeId":id},
            callback:(res)=>{
              if(res.code===200){
    ...
    setClassNotice = (type,id) => {
      if(type==1){
        this.props.dispatch(routerRedux.push("/class-score-type-detail?type=1"))
      }else{
        this.props.dispatch(routerRedux.push("/class-score-type-detail?type=2&id="+id))
      }
    }
    render(){
        const columns = [{
