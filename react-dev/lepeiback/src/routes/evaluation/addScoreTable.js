...
class addScoreTable extends Component{
    ...
    onPageChange=(current,size)=>{
        ...
        this.props.form.validateFields((err, values) => {
            me.setState({page: current, prePage: size})
            if(values.cycle==1){
                let params = {
                                ...
                                i.types.map(e=>{
                                    ...
                                    cls.typeCalculate.map(k=>{
                                        if(k.typeId==e.typeId){
                                            e.typeTotalScore = k.typeTotalScore
                                            e.typeAverageScore = k.typeAverageScore
                                        }
                                    })
                                })
                            })
                        ...
                        {getFieldValue('cycle')==2?<Col span={8}>
                                                        <FormItem >
                                                            {getFieldDecorator("weeks",{rules: [{
                                                                ...
                                                                <Select width={380} placeholder="请选择时间" mode="multiple" optionFilterProp="children">
                                                                    {weekChild}
                                                                </Select>
                                                            )}
                                                        </FormItem>
                                    ...
                                    j.cates.map((h, hIndx)=>
                                        h.types.map((l, lIndex)=>
                                            <tr key={lIndex}>
                                                ...
                                                {lIndex==0&& hIndx === 0?<td rowSpan={j.col}>{j.className}</td> :null}
                                                ...
                                                <td>{l.typeTotalScore?l.typeTotalScore:"-"}</td>
                                                <td>{l.typeAverageScore?l.typeAverageScore:"-"}</td>
                                                {lIndex==0&& hIndx === 0?<td rowSpan={j.col}>{j.totalScore}</td> :null}
                                                {lIndex==0&& hIndx === 0?<td rowSpan={j.col}>{j.averageScore}</td> :null}
                                            </tr>
                                    )
                                    )
                                )
