...
class addEvaluationType3 extends Component{
    constructor(props) {
        super(props);
        this.state = {
    ...
    changeDefaultScore=(index,e)=>{
      let newData = this.state.secondTypes
      newData[index].defaultScore =  e.target.value
      this.setState({
        secondTypes: newData
      })
    }

    changeReduceScore=(index,e)=>{
      let newData = this.state.secondTypes
      newData[index].reduceScore =  e.target.value
      this.setState({
        secondTypes: newData
      })
    }
    ...
    save = () =>{
      this.props.form.validateFields(['sort', 'cycleType', 'name', 'totalScore', 'marks', 'padExaminers', 'teacherExaminers'], (err, values) => {
        if(err) return
        let secondTypes = this.state.secondTypes
        let flag = false
        secondTypes.map(i=>{
          if(!i.reduceScore||!i.defaultScore||!i.name||!i.totalScore){
            flag = true
          }
        })
    ...
    add=()=>{
      ...
      this.props.form.resetFields(['kw'])
    }
    handleCancel=()=>{
      this.setState({
        visible: false
                ...
                </Row>
                {
                    secondTypes.map((i, index)=>{
                      return <Row gutter={24} key={index}>
                                ...
                                <Col span={5}>
                                    <Form.Item {...formItemLayout1} label="分值">
                                      <Input maxLength={5} disabled={getQueryString("id")?"disabled":false} placeholder="请输入" value={i.totalScore} onChange={this.changeScore.bind(this, index)}/>
                                    </Form.Item>
                                </Col>
                                <Col span={5}>
                                    <Form.Item {...formItemLayout1} label="扣分点分值" >
                                      <Input maxLength={5} disabled={getQueryString("id")?"disabled":false} placeholder="请输入" value={i.reduceScore} onChange={this.changeReduceScore.bind(this, index)}/>
                                    </Form.Item>
                                </Col>
                                <Col span={5}>
                                    <Form.Item {...formItemLayout1} label="默认分" >
                                      <Input maxLength={5} disabled={getQueryString("id")?"disabled":false} placeholder="请输入" value={i.defaultScore} onChange={this.changeDefaultScore.bind(this, index)}/>
                                    </Form.Item>
                                </Col>
                                <Col span={4}>
                                  {!getQueryString("id")?<Button type="link" onClick={this.remove.bind(this, index)}>删除</Button>:null}
                                </Col>
                          </Row>
                    })
                  }
                  ...
                  <Row gutter={24}>
                      <Col span={24}>
                        <Form.Item {...formItemLayout2} label="app(学生)">
                          ...
                          {/* {getFieldDecorator('studentExaminers', {
                            rules: [{ required: true, message: '请选择' }],
                          })(
                             */}
                            <div>
...
export default connect(mapStateToProps)(Form.create()(addEvaluationType3));
